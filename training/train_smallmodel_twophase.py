"""
Two-Phase SmallModel Training (12M params)
==========================================
...
For kWave: uses only 4,600 hyperechoic samples (not all 13,800).
For Field II: uses all 4,600 samples (1 echo type).

Usage:
    python train_smallmodel_twophase.py --sim_type fieldii --train_caps 200,300,400,500
    python train_smallmodel_twophase.py --sim_type kwave --train_caps 200,300,400,500
"""
...
WANDB_PROJECT="elastography"; P1_SAVE_DIR=f"model/best_smallmodel_{SIM_TYPE}"
...
def main():
    ...
    # ── PHASE 1 ──
    wandb.init(project=WANDB_PROJECT,name=f"smallmodel_p1_{SIM_TYPE}",config={"model":"FullModelSmall (12M)","sim_type":SIM_TYPE,"p1_epochs":PHASE1_EPOCHS,"kwave_hyper_only":SIM_TYPE=="kwave"})
    ...
    print("\nP1 synth test..."); run_test(model,ste_l,device,f"results/smallmodel_{SIM_TYPE}_p1_synth","P1 Synthetic",epoch=bep1)
    ...
    for cap in TRAIN_CAPS:
        rn=f"smallmodel_{SIM_TYPE}_p2_{cap}pt"
        print(f"\n{'#'*60}\n  P2: {SIM_TYPE} -> {cap} clinical\n{'#'*60}")
        wandb.init(project=WANDB_PROJECT,name=rn,reinit=True,config={"model":"FullModelSmall (12M)","sim_type":SIM_TYPE,"p1_checkpoint":p1ckpt,"train_cap":cap,"p2_epochs":PHASE2_EPOCHS,"kwave_hyper_only":SIM_TYPE=="kwave"})
        ...
        model=FullModelSmall().to(device); model.load_state_dict(torch.load(p1ckpt,map_location=device,weights_only=True))
        print(f"Loaded P1: {p1ckpt}")
        print("\n--- Before FT ---")
        run_test(model,tl_l,device,f"results/smallmodel_{SIM_TYPE}_{cap}pt_before_ft","P1 on Clinical (no FT)",epoch=0,test_files=tf)
        if os.path.isdir("data/ClinicalData"):
            from utils.predict_clinical_pairs import predict_clinical_pairs
            predict_clinical_pairs(model,device,clinical_folder="data/ClinicalData",mask_folder=clinical_mask_folder,save_dir=f"results/smallmodel_{SIM_TYPE}_{cap}pt_pairs_before",wandb_label="ClinicalData (before FT)",log_wandb=True)
        ...
        bv2,bep2=float('inf'),0; smd=f"model/best_smallmodel_{SIM_TYPE}_{cap}pt"; os.makedirs(smd,exist_ok=True)
        ...
        for ep in range(1,PHASE2_EPOCHS+1):
            ...
            if ep%10==0 or ep==1:
                ...
                hd=f"results/smallmodel_{SIM_TYPE}_{cap}pt_hist"; os.makedirs(hd,exist_ok=True); log_all_histograms(em,"p2/during_training",ep,hd)
        ...
        run_test(model,tl_l,device,f"results/smallmodel_{SIM_TYPE}_{cap}pt_final",f"Clinical ({SIM_TYPE} {cap}pt)",epoch=bep2,test_files=tf)
        if os.path.isdir("data/ClinicalData"):
            predict_clinical_pairs(model,device,clinical_folder="data/ClinicalData",mask_folder=clinical_mask_folder,save_dir=f"results/smallmodel_{SIM_TYPE}_{cap}pt_pairs_after",wandb_label="ClinicalData (after FT)",log_wandb=True)
